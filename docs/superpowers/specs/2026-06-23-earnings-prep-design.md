...
- **Edit** (default): Story cards — title + question preview, expandable to reveal its metrics — plus a "+ Add Story" button and per-story edit/delete icons. Delete reuses the existing `ConfirmDialog` component (the same pattern already used for deleting holdings/transactions elsewhere in the app).
...
- Creating Stories for tickers not currently held in the portfolio.
